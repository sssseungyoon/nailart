...
export function Waves({
  ...
  return (
    ...
    >
      <svg
        ref={svgRef}
        className="pointer-events-none block w-full h-full js-svg"
      ...
      {(title || subtitle || ctaText) && (
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
          ...
          {subtitle && (
            <p className="text-white/60 text-lg md:text-xl mt-4">{subtitle}</p>
          )}
          {ctaText && (
            <a href="/auth">
              <button
                onClick={onCtaClick}
                className="pointer-events-auto mt-8 px-8 py-3 border border-white text-white bg-transparent rounded-full text-sm md:text-base tracking-wide transition-colors duration-200 hover:bg-white hover:text-black cursor-pointer"
              >
                {ctaText}
              </button>
            </a>
